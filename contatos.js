...
const createContact = async (contato) => {
    const url = "http://localhost/SENAI/backend/apiphp/contatos/"
    const options = {
        method: 'POST',
...
const updateContact = async (contact) => {
    const url = `http://localhost/SENAI/backend/apiphp/contatos/${contact.id}`
    const options = {
        method: 'PUT',
...
const editContact = async() => {
    ...
    if (id > 0) {
        const url = `http://localhost/SENAI/backend/apiphp/contatos/${id}`
...
const deleteContact = async () => {
    ...
    if (idContact > 0) {
        const url = `http://localhost/SENAI/backend/apiphp/contatos/${idContact}`
        const options = {
            method: 'DELETE',
        }
        await fetch(url, options)
